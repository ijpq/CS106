...
string TextRender::font() const {
    return mFont;
}

string TextRender::computedFont() const {
    ...
    while (true) {
        ...
        for (size_t i = 0; i < strings.size(); i++) {
            ...
            auto render = TextRender::construct(strings[i], {
                                                ...
                                                }, textColors[i], currFont, breakMode);
...
shared_ptr<LegendRender> LegendRender::construct(const vector<string>& strings,
                                                 ...
                                                 const string& borderColor,
                                                 LineBreak breakMode) {
    return construct(strings, colors, bounds, vector<string>(strings.size(), textColor), font, borderColor, breakMode);
}

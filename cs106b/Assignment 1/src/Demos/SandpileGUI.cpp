...
namespace {
    ...
    const string kLegendFont  = "Monospaced-BOLD-14";
    const string kLegendColor = "#343434"; // Jet
    ...
    void drawLegend(int grainsDropped, GWindow& window) {
        GText label(addCommasTo(grainsDropped) + (grainsDropped == 1? " sand grain dropped." : " sand grains dropped."));
        label.setFont(kLegendFont);
        label.setColor(kLegendColor);
